...
const useTabPdfBtn = document.getElementById("use-tab-pdf");
...
chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    if (isLocalPdfUrl(url)) {
        ...
        if (tabPdfWhy)
            tabPdfWhy.textContent = "This is a local PDF. Use \"Choose PDF file\" below and select the same file to stamp it.";
        if (localHint) {
            localHint.classList.remove("hidden");
            localHint.hidden = false;
        }
        if (tipNoPdf)
            tipNoPdf.classList.add("hidden");
    }
    else {
        ...
        if (pdfUrl) {
            ...
            if (tabPdfWhy)
                tabPdfWhy.textContent = "Click to open the Bates viewer with this tab's PDF.";
            if (tipNoPdf)
                tipNoPdf.classList.add("hidden");
        }
        else {
            ...
            delete useTabPdfBtn.dataset.localPdf;
            if (tabPdfWhy)
                tabPdfWhy.textContent = "With a web PDF in this tab, click the button above.";
            if (tipNoPdf)
                tipNoPdf.classList.remove("hidden");
        }
    }
});
